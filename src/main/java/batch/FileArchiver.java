...
public String process() throws Exception  // test comment lines
{
    try{
        Properties jobParameters = BatchRuntime.getJobOperator().getParameters(jobCtx.getExecutionId());
            LOG.debug("Archive files in directory for job " + jobCtx.getExecutionId());
...
catch (Exception e)
{
    ...
    {
         try {
            LOG.debug(" ... entering zipIt with : " + zipFile + "  " + fileList.toString());
        byte[] buffer = new byte[1024];
            ...
            zos.closeEntry();
            zos.close();
                LOG.debug("Done with creating the file archive");
