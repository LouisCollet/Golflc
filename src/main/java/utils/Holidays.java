...
public class Holidays {
 ...
 // vérifier si la date est un jour férié
  ...
  public boolean CountryHolidays(LocalDate ld, String country) {
            LOG.debug("entering CountryHolidays");
            LOG.debug("localdate = " + ld);
...
hol.forEach((k,v) -> LOG.debug("Holidays Belgium = Holiday : " + v + " / Date : " + k));
//SortedSet<String> keys = new TreeSet<String>(hol.keySet());
if (hol.containsKey(lda) ) {
    LOG.debug("Round played  Holiday : " + hol.get(lda)+ " /" + slda);
     return true;
}else{
    LOG.debug("is NOT an holiday : " + slda);
  ...
  try{
        boolean b = false;
        String slda = lda.format(ISO_LOCAL_DATE); // yyyy/MM/dd
            LOG.debug("entering CountrySpain with localdate = " + lda);
            LOG.debug("converted to String = = " + slda);
        int year = lda.getYear();
         LocalDate easter = utils.LCUtil.EasterSundayDate(year);
            LOG.debug("easter = " + easter);
...
hol.forEach((k,v) -> LOG.debug("Holidays Spain = Holiday : " + v + " / Date : " + k));

if (hol.containsKey(lda) ) {
    LOG.debug("Hash map contains lda : " + hol.get(lda)+ " /" + slda);
     return true;
}else{
    LOG.debug("is NOT an holiday : " + slda);
    return false; // is not an holiday
 ...
 try{
        ...
        int year = lda.getYear();
        String slda = lda.format(ISO_LOCAL_DATE); // yyyy/MM/dd
            LOG.debug("entering CountryGermany with localdate = " + lda);
            LOG.debug("converted to String = = " + slda);
...
hol.forEach((k,v) -> LOG.debug("Holidays Germany = Holiday : " + v + " / Date : " + k));
//SortedSet<String> keys = new TreeSet<String>(hol.keySet());
if (hol.containsKey(lda) ) {
    LOG.debug("Round played  Holiday : " + hol.get(lda)+ " /" + slda);
     return true;
 ...
 try{
        ...
        int year = lda.getYear();
        String slda = lda.format(ISO_LOCAL_DATE); // yyyy/MM/dd
            LOG.debug("entering CountryNederland with localdate = " + lda);
            LOG.debug("converted to String = = " + slda);
         LocalDate easter = utils.LCUtil.EasterSundayDate(year);
            LOG.debug("easter = " + easter);
         Map<LocalDate,String> hol = new LinkedHashMap<>();
...
hol.forEach((k,v) -> LOG.debug("Holidays Nederland = Holiday : " + v + " / Date : " + k));
//SortedSet<String> keys = new TreeSet<String>(hol.keySet());
if (hol.containsKey(lda) ) {
    LOG.debug("Round played  Holiday : " + hol.get(lda)+ " /" + slda);
     return true;
...
/*
      ...
      //https://gist.github.com/bdkosher/9414748   tableau pour vérifier          
        ...
        boolean b = false;
    //    String slda = lda.format(ISO_LOCAL_DATE); // yyyy/MM/dd
     //       LOG.debug("entering CountryUSA with localdate = " + lda);
       //     LOG.debug("converted to String = " + slda);
...
hol.forEach((k,v) -> LOG.debug("Holidays USA = Holiday : " + v + " / " + k));
if (hol.containsKey(lda) ) {
    LOG.debug("Is an HOLIDAY !! : " + hol.get(lda)+ " /" + lda);
     return true;
}else{
    LOG.debug("is NOT an holiday : " + lda);
    return false; // is not an holiday
}
 } catch (Exception e) {
...
/// if a holiday falls on a Sunday it is celebrated the following Monday. 
       //     LOG.debug("entering FixWeekEnd");
             LOG.debug("entering FixWeekEnd with : " + hol);
            DayOfWeek dayOfWeek = hol.getDayOfWeek();
    //        LOG.debug("dayOfWeek = " + dayOfWeek);
            switch(dayOfWeek) {
                    case SATURDAY:
                         hol = hol.minusDays(1);
                         LOG.debug("holiday MOVED from Saturday to Friday : " + hol);
                         break;
                    case SUNDAY:
                         hol = hol.plusDays(1);
                         LOG.debug("holiday MOVED from Sunday to monday : " + hol);
                        break;
                    default:
             //           LOG.debug("no change !!= " + dayOfWeek.name());
                        break;
            }
          return hol;
        }
 ...
 public static int dateYear(int year){
  ...
  void main() throws ParseException, SQLException {
  try{
