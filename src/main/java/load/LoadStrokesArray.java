...
{
    LOG.info("starting LoadPointsArray with par = " + Arrays.toString(points) );
    ...
    rs.beforeFirst(); //  Initially the cursor is positionned before the first row
      ...
      while (rs.next())
        {rowNum = rs.getRow() - 1;
           ...
           LOG.info(" -- points = " + rowNum + " / " + points[rowNum][3] );
        } // end while
...
//    {
        LOG.info(" -- ending getHoleStrokes : = " + Arrays.deepToString(points) );
//    } 
return points;
}
...
{
    LOG.error("NullPointerException in getHoletrokes " + npe);
