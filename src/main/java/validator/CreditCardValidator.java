...
public class CreditCardValidator implements ConstraintValidator<CreditCardV, String>{
int max;  //added 1/11/2016

@Override
public void initialize(CreditCardV firstUpper){
    max = firstUpper.max();
//    LOG.debug("CreditCard Validator with input max = " + max);
...
public boolean isValid(String value, ConstraintValidatorContext context){
  //  LOG.debug("entering isValid");
   // LOG.debug("entering isValid with value = " + value);
   
    	if (value == null || value.length() == 0){
   //         LOG.debug(" validateCreditCardNumber : card NUMBER is null");
	return false;
	}
    value = value.replaceAll(" ", "");
 //   LOG.debug("entering isValid with SPACES removed = " + value);//return value.substring(0, 1).equals(value.substring(0, 1).toUpperCase());
    return validateCreditCardNumber(value);
}

private boolean validateCreditCardNumber(String str) {
 //   LOG.debug("entering validateCreditCardNumber");
 //   LOG.debug("entering validateCreditCardNumber with input = " + str);
		...
		if (sum % 10 == 0) {
   //                 LOG.debug(" validateCreditCardNumber : card OK");
			return true;
		} else {
   //                  LOG.debug(" validateCreditCardNumber : card INVALID NUMBER");
			return false;
		}
	} //end method
